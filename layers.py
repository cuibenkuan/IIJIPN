...
def dot(x, y, sparse=False):
    ...
    if sparse:
        ...
        res = tf.einsum('bij,jk->bik', x, y)
    return res


def intra_propagations(i, support, x, var, act, mask, dropout, sparse_inputs=False):
...
def inter_propagation(var, support, support1, support2):
...
class GraphLayer(Layer):
    ...
    def __init__(self, input_dim, output_dim, placeholders, dropout=0.,
        ...
        with tf.variable_scope(self.name + '_vars'):
            ...
            for i in range(3):
                ...
                self.vars['bias_' + str(i) + '_z0'] = zeros([output_dim], name='bias_' + str(i) + '_z0')
    ...
    def _call(self, inputs):
        ...
        if self.sparse_inputs:
            ...
            xx = tf.nn.dropout(xx, 1 - self.dropout)
        ...
        i = 1
        x = dot(xx, self.vars['weights_encode_' + str(i)],
        ...
        for _ in range(self.steps):
            support1 = intra_propagations(i, self.adj1, support1, self.vars, self.act,
                                          self.mask1, 1 - self.dropout, self.sparse_inputs)
        ...
        for _ in range(self.steps):
            support2 = intra_propagations(i, self.adj2, support2, self.vars, self.act,
                                          self.mask2, 1 - self.dropout, self.sparse_inputs)

        supports = inter_propagation(self.vars, support, support1, support2)

        supports = attentin_graps(supports, self.vars)
